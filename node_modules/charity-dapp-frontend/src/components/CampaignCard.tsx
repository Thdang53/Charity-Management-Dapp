...
export function CampaignCard({ campaign, onDonate, onWithdraw, currentAddress }: CampaignCardProps) {
  ...
  // 🔥 Check nếu campaign là optimistic (chưa có object ID thật từ blockchain)
  // Sui object ID luôn là 32 bytes = 64 hex chars + "0x" prefix = 66 chars
  // TxDigest cũng 66 chars, nhưng chỉ có khi vừa tạo (chưa load từ blockchain)
  // Cách check tốt nhất: Xem campaign có từ blockchain hay từ optimistic update
  const isOptimistic = !campaign.id.startsWith('0x') || campaign.id.length !== 66;
  const canDonate = isActive && !isOptimistic;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
        {isOptimistic ? (
          // Campaign đang chờ blockchain confirm
          <div style={{
            flex: 1,
            padding: '12px',
            borderRadius: '8px',
          ...
          <>
            <button
              ...
              style={{
                ...
                backgroundColor: canDonate ? '#2196f3' : '#e0e0e0',
                color: canDonate ? '#fff' : '#9e9e9e',
                transition: 'all 0.2s',
              }}
              onMouseEnter={(e) => {
                if (canDonate) {
      ...
      {isOptimistic && (
        <div style={{
          ...
          ID: {campaign.id.slice(0, 10)}...{campaign.id.slice(-8)}
        </div>
      )}
    </div>
  );
